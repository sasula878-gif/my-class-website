...
# Код приглашения для одноклассников
...
def init_db():
    ...
    # Добавляем тебя и учителей (смени 'твой_пароль' на настоящий!)
    admins = [
...
def login():
    if request.method == 'POST':
        ...
        if user:
            ...
            conn.commit()
            conn.close()
            return f"Успех! Логин: <b>{login_input}</b>, Пароль: <b style='color:red;'>{new_pwd}</b>. <a href='/login'>Войти</a>"
        
        return "Ошибка данных. <a href='/login'>Назад</a>"
